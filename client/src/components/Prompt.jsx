...
export default function Prompt({generateImg, handelPrompt, prePrompt, prompt, generating}) {
    return (
        <form onSubmit={generateImg} className="w-[85%] max-[640px]:w-full flex justify-center items-center gap-4 max-[640px]:flex-col max-[640px]:px-4">
            ...
            <button type="submit" className="py-4 max-[640px]:py-3 px-6 max-[640px]:w-full text-center bg-[var(--oil-clr)] rounded-md disabled:cursor-not-allowed disabled:bg-[var(--oil-clr-drk)]" disabled={generating}>{ generating ? "Generating" : "Generate" }</button>
        </form>
    )
}
